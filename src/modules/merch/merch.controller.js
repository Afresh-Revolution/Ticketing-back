...
export async function adminUpdateOrderStatus(req, res, next) {
  try {
    ...
    const allowed = await merchModel.merchOrderAccessibleByAdmin(
      req.params.id,
      req.user.id,
      req.user.role
    );
...
export async function adminDeleteOrder(req, res, next) {
  try {
    ...
    const deleted = await merchModel.deleteMerchOrder(orderId);
    if (!deleted) return res.status(404).json({ error: 'Order not found' });

    res.json({ message: 'Merch order deleted', id: orderId });
  } catch (e) {
    next(e);
  }
}
