...
export async function incrementMerchStock(client, lines) {
  for (const line of lines) {
    if (!line.imageId) continue;
    await client.query(
      `UPDATE event_merch_images
...
export async function merchOrderAccessibleByAdmin(orderId, userId, role) {
  if (!(await merchTablesReady())) return false;
  const isSuper = role === 'superadmin' || userId === 0 || userId === '0';
  if (isSuper) {
    const { rows } = await query('SELECT 1 FROM merch_orders WHERE id = $1', [orderId]);
    return rows.length > 0;
  }
  if (userId == null) return false;
  const { rows } = await query(
    `SELECT 1 FROM merch_orders mo
     JOIN "Event" e ON e.id::text = mo.event_id::text
     WHERE mo.id = $1 AND e."createdBy"::text = $2`,
...
export async function updateMerchOrderStatus(orderId, status, paystackReference) {
  ...
  try {
    ...
    const stockLines = itemsRes.rows.map((r) => ({
      imageId: r.image_id,
      quantity: r.quantity,
    }));

    if (status === 'paid' && prev.status !== 'paid') {
      await decrementMerchStock(client, stockLines);
    } else if (prev.status === 'paid' && status !== 'paid') {
      await incrementMerchStock(client, stockLines);
    }
...
export async function deleteMerchOrder(orderId) {
  ...
  try {
    ...
    if (prev.status === 'paid') {
      ...
      await incrementMerchStock(
        client,
        itemsRes.rows.map((r) => ({ imageId: r.image_id, quantity: r.quantity }))
      );
    }
    await client.query('DELETE FROM merch_orders WHERE id = $1', [orderId]);
    await client.query('COMMIT');
    return true;
  } catch (e) {
    await client.query('ROLLBACK');
